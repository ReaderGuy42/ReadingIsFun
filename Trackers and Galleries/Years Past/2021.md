...
`button-bookgallery` 
```dataviewjs
let BookCount = (dv.pages('"2 Book Log/2021"').where(p => p.Medium == "Book" || p.Medium == "eBook").length);

let AudioCount = (dv.pages('"2 Book Log/2021"').where(p => p.Medium == "Audiobook" || p.Medium == "Podcast").length);

let pagesum = 0;
   for(let i = 0; i < dv.pages('"2 Book Log/2021"').where(p => p.Medium == "Book" || p.Medium == "eBook").length; i++) {
     if(dv.pages('"2 Book Log/2021"').where(p => p.Medium == "Book" || p.Medium == "eBook")[i].Length) {
       pagesum += dv.pages('"2 Book Log/2021"').where(p => p.Medium == "Book" || p.Medium == "eBook")[i].Length;
         }
   }

let hoursum = 0;
	for(let i = 0; i < dv.pages('"2 Book Log/2021"').where(p => p.Medium == "Audiobook" || p.Medium == "Podcast").length;i++) {
     if(dv.pages('"2 Book Log/2021"').where(p => p.Medium == "Audiobook" || p.Medium == "Podcast")[i].Length) {
       hoursum += dv.pages('"2 Book Log/2021"').where(p => p.Medium == "Audiobook" || p.Medium == "Podcast")[i].Length;
         }
   }

let Grade = 0;
	for(let i = 0; i < dv.pages('"2 Book Log/2021"').length;i++) {
     if(dv.pages('"2 Book Log/2021"')[i].Rating) {
       Grade += dv.pages('"2 Book Log/2021"')[i].Rating;
         }
   }
let AvgRating = (Grade / (dv.pages('"2 Book Log/2021"').where(p => p.Medium == "Book" || p.Medium == "eBook" || p.Medium == "Audiobook" || p.Medium == "Podcast").length))

let today = DateTime.now().toFormat("ooo");

dv.paragraph("#### In 2021 I read " + BookCount + " books and I've listened to " + AudioCount + " audiobooks (of which " + dv.pages('"2 Book Log/2020"').where(p => p.Medium == "Podcast").length + " were podcasts), for an overall " + (BookCount + AudioCount) + " books and audiobooks read.")

dv.paragraph("#### I read " + pagesum + " pages this year. And I've listened to " + (hoursum.toFixed(1)) + " hours of audiobooks and podcasts.")
