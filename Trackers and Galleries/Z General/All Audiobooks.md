...
``` dataviewjs
	...
	for(
		let results of dv.pages('"2 Book Log"')
			.where(p => p.DateFinished && p.Length && p.Medium == "Audiobook" || p.Medium == "Podcast") //or p.DateFinished && p.Pages
			...
			let lbl = "Hours listened in " + results.key;
...
```dataviewjs
let BookCount = (dv.pages('"2 Book Log"').where(p => p.Medium == "Book" || p.Medium == "eBook").length);

let AudioCount = (dv.pages('"2 Book Log"').where(p => p.Medium == "Audiobook" || p.Medium == "Podcast").length);

let pagesum = 0;
   for(let i = 0; i < dv.pages('"2 Book Log"').where(p => p.Medium == "Book" || p.Medium == "eBook").length; i++) {
     if(dv.pages('"2 Book Log"').where(p => p.Medium == "Book" || p.Medium == "eBook")[i].Length) {
       pagesum += dv.pages('"2 Book Log"').where(p => p.Medium == "Book" || p.Medium == "eBook")[i].Length;
         }
   }
...
dv.paragraph("# Since 2020 I've listened to " + AudioCount + " audiobooks of which " + dv.pages('"2 Book Log"').where(p => p.Medium == "Podcast").length + " were podcasts.")

dv.paragraph("# I listened to " + (hoursum.toFixed(1)) + " hours.")
